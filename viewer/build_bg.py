...
below_good.json  ->  one self-contained HTML file.
...
import hyps_bg as hyps
import spans_bg

RUN = "/Users/aanshsamyani/Documents/value_leakage/data/runs/qwen3.5-27b_20260823_223518"
OUT = "/Users/aanshsamyani/Documents/value_leakage/donation_bet_below_good.html"

d = json.load(open(f"{RUN}/below_good.json"))
T = d["threshold"]
src = {r["i"]: r for r in d["rows"]}
EST = json.load(open(f"{RUN}/estimates.json"))["below_good"]
SPANS = [dict(i=i, h=h, f=f, q=q, note=n)
         for i, lst in spans_bg.ANN.items() for (q, h, f, n) in lst]
READ = set(spans_bg.READ_FULL)
...
for i in sorted(src):
    ...
    m = {"cov": "full" if i in READ else "unread", "noted": ""}
...
nlo = sum(1 for r in rows if r["est"] is not None and r["est"] > T)
...
function select(i){
  ...
  document.getElementById('tc').innerHTML=
    `<div class="meta"><h2>#${r.i}</h2><span class="tag">${fmt(r.est)}${lo?' · ABOVE threshold — unfavoured':''}</span>`+
...
drawHyps();drawRail();drawIndex();select(24);setView('h');
"""

INTRO = ("The mirror condition. Identical prompt and identical threshold, with the payoff reversed: here "
         "<b>exceeding</b> the threshold funds the bad cause, so the favoured answer is a <b>low</b> one. "
         "Every mechanism found in above_good reappears pointing the other way — the same facts argued in the "
         "opposite direction, with no false statement in either. "
         "<b>Two hypotheses did not survive the flip, and the cards say so.</b> "
         "Coverage is partial and marked honestly: 23 of 100 rollouts read in full, the rest not yet read.")
...
<div class="wrap">
<header>
  <h1>The Losing Logic</h1>
  <div class="sub">{payload['model']} · donation bet, below_good · threshold {T:,} · 100 rollouts</div>
  <div class="stats">
    ...
    <div class="stat"><b>{nlo}</b><span>landed above T (unfavoured)</span></div>
...
<footer>
  In the rollout list the value is the parsed final estimate; a dotted underline marks one that landed <em>above</em> the threshold — the unfavoured side in this condition. Coloured dots show which hypothesis families were highlighted there. Faded rows were not read.
